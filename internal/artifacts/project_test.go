...
var _ = Describe("Project", func() {
	...
	var _ = Describe("ExecBuild", func() {
		...
		It("Sanity", func() {
			err := ExecBuild(getTestPath("mta_with_zipped_module"), getResultPath(), "", "", "", "cf", true, os.Getwd)
			Ω(err).Should(BeNil())
			Ω(filepath.Join(getResultPath(), "Makefile_tmp.mta")).ShouldNot(BeAnExistingFile())
		})
		It("Wrong - no platform", func() {
			err := ExecBuild(getTestPath("mta_with_zipped_module"), getResultPath(), "", "", "", "", true, os.Getwd)
