...
`)
		...
		It("Sanity - Dev", func() {
			Ω(makeFile(&fs.Loc{SourcePath: filepath.Join(wd, "testdata"), Descriptor: "dev"}, makeFileName, &tpl)).Should(Succeed())
			Ω(makeFileFullPath).Should(BeAnExistingFile())
			Ω(getMakeFileContent(makeFileFullPath)).Should(Equal(expectedMakeFileContent))
		})
		It("Sanity - Dep", func() {
			Ω(makeFile(&fs.Loc{SourcePath: filepath.Join(wd, "testdata"), Descriptor: "dep"}, makeFileName, &tplDep)).Should(Succeed())
			Ω(makeFileFullPath).Should(BeAnExistingFile())
		...
		It("Make testing with wrong mta yaml file", func() {
			Ω(Make(&fs.Loc{SourcePath: filepath.Join(wd, "testdata"), MtaFilename: "xxx.yaml"}, "")).Should(HaveOccurred())
		})
		It("Make testing with wrong mode", func() {
			Ω(Make(&fs.Loc{SourcePath: filepath.Join(wd, "testdata")}, "wrongMode")).Should(HaveOccurred())
		})
	})
	...
	var _ = DescribeTable("Makefile Generation Failed", func(testPath string, testTemplateFilename string) {
		...
		ep := fs.Loc{SourcePath: filepath.Join(wd, "testdata")}
