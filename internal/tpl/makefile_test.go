...
`)
		...
		var _ = Describe("ExecuteMake", func() {
			...
			It("Sanity", func() {
				Ω(ExecuteMake(filepath.Join(wd, "testdata"), filepath.Join(wd, "testdata"), makefile, "dev", "", os.Getwd)).Should(Succeed())
				Ω(filepath.Join(wd, "testdata", "Makefile.mta")).Should(BeAnExistingFile())
			})
			It("Fails on location initialization", func() {
				Ω(ExecuteMake("", filepath.Join(wd, "testdata"), makefile, "dev", "", func() (string, error) {
					return "", errors.New("err")
				})).Should(HaveOccurred())
			})
			It("Fails on wrong mode", func() {
				Ω(ExecuteMake(filepath.Join(wd, "testdata"), filepath.Join(wd, "testdata"), makefile, "dev", "wrong", os.Getwd)).Should(HaveOccurred())
			})
		})
