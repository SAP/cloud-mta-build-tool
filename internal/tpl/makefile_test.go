...
`)
		...
		Describe("ExecuteMake", func() {
			...
			It("Sanity", func() {
				Ω(ExecuteMake(filepath.Join(wd, "testdata"), filepath.Join(wd, "testdata"), nil, makefile, "", os.Getwd, true, true)).Should(Succeed())
			...
			It("Fails on location initialization", func() {
				...
				}, true, true)).Should(HaveOccurred())
			})
			It("Fails on wrong mode", func() {
				Ω(ExecuteMake(filepath.Join(wd, "testdata"), filepath.Join(wd, "testdata"), nil, makefile, "wrong", os.Getwd, true, true)).Should(HaveOccurred())
			})
		})
		...
		It("Sanity", func() {
			...
			Ω(makeFile(&ep, &ep, &ep, nil, makeFileName, &tpl, true, true)).Should(Succeed())
		...
		It("genMakefile testing with wrong mta yaml file", func() {
			...
			Ω(genMakefile(&ep, &ep, &ep, &ep, nil, makefile, "", true, true)).Should(HaveOccurred())
		})
		...
		It("genMakefile testing with wrong mode", func() {
			...
			Ω(genMakefile(&ep, &ep, &ep, &ep, nil, makefile, "wrongMode", true, true)).Should(HaveOccurred())
		})
		...
		DescribeTable("genMakefile should fail when there is a circular build dependency between modules", func(mode string) {
			...
			Ω(genMakefile(&ep, &ep, &ep, &ep, nil, makefile, mode, true, true)).Should(HaveOccurred())
		},
	...
	DescribeTable("Makefile Generation Failed", func(testPath string, testTemplateFilename string) {
		...
		Ω(makeFile(&ep, &ep, &ep, nil, makeFileName, &tplCfg{relPath: testPath, tplContent: testTemplate, preContent: basePreVerbose, postContent: basePost, depDesc: "dev"}, true, true)).Should(HaveOccurred())
	},
