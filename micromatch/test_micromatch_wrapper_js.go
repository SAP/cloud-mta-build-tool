...
func test_micromatch_help() {
	fmt.Println("Testing micromatch-wrapper-js!!")

	out, err := exec.Command("node", "micromatch-wrapper.js", "-h").Output()
	if err != nil {
...
func test_micromatch_match(files, patterns []string) {
	...
	if strings.Contains(output, "Not Match") {
		// Skip
	} else if strings.Contains(output, "Match Files") {
		// copy files to MTA Archive
	}
	fmt.Println()
}
...
func main() {
	...
	target = filepath.Join(cwd, "node-js/export-result-file")
	patterns = []string{}
	...
	source = filepath.Join(cwd, "node-js")
	target = filepath.Join(cwd, "node-js/export-result-file")
	patterns = []string{"node_modules/**", "!node_modules/prune/**"}
	test_micromatch_getPackagedFiles(source, target, patterns)
