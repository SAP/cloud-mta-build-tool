    ...
    if (options.followRedirects) {
        return _request(method, url, {
            ...
            if (options.followRedirects && isRedirect(res.statusCode)) {
                ...
                options = __assign({}, options, { maxRedirects: options.maxRedirects && options.maxRedirects !== Infinity ? options.maxRedirects - 1 : options.maxRedirects });
                ...
                if (typeof location_1 !== 'string') {
                    return callback(new Error('Cannot redirect to non string location: ' + location_1));
                }
                return request(duplex ? 'GET' : method, url_1.resolve(url, location_1), options, callback);
            }
            else {
        ...
        return cache.getResponse(url, function (err, cachedResponse) {
            ...
            request('GET', url, {
                ...
                if (options.canCache ? options.canCache(res, canCache) : canCache) {
                    var cachedResponseBody = new stream_1.PassThrough();
                    var resultResponseBody = new stream_1.PassThrough();
