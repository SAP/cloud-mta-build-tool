...
var _ = Describe("ModulesDeps", func() {
	...
	It("Resolve dependencies - Valid case", func() {
		wd, _ := os.Getwd()
		mtaStr, _ := ParseFile(&Loc{SourcePath: filepath.Join(wd, "testdata"), MtaFilename: "mta_multiapps.yaml"})
	...
	It("Resolve dependencies - cyclic dependencies", func() {
		wd, _ := os.Getwd()
		mtaStr, _ := ParseFile(&Loc{SourcePath: filepath.Join(wd, "testdata"), MtaFilename: "mta_multiapps_cyclic_deps.yaml"})
