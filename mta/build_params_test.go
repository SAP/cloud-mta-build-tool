...
var _ = Describe("BuildParams", func() {
	...
	var _ = DescribeTable("validateArtifacts", func(ep dir.EndPoints, requiredModule Modules, artifacts []string, match GomegaMatcher) {

		Ω(validateArtifacts(ep, requiredModule, artifacts)).Should(match)

	},
		Entry("Valid artifacts", dir.EndPoints{}, Modules{}, []string{"*"}, Succeed()),
		Entry("Invalid artifacts", dir.EndPoints{}, Modules{}, []string{"*", "."}, HaveOccurred()),
	)

	var _ = DescribeTable("getBuildResultsPath", func(module Modules, expected string) {
		Ω(module.getBuildResultsPath(dir.EndPoints{})).Should(HaveSuffix(expected))
	},
	...
	var _ = DescribeTable("getRequiredTargetPath", func(requires BuildRequires, module Modules, expected string) {
		Ω(requires.getRequiredTargetPath(dir.EndPoints{}, &module)).Should(HaveSuffix(expected))
	},
	...
	var _ = Describe("ProcessRequirements", func() {
		...
		var _ = DescribeTable("Invalid cases", func(require BuildRequires, mtaObj MTA, moduleName string) {
			Ω(require.ProcessRequirements(dir.EndPoints{}, mtaObj, moduleName)).Should(HaveOccurred())
		},
